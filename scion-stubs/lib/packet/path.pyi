...
class SCIONPath(Serializable, Sized):
    ...
    def get_iof(self) -> Optional[InfoOpaqueField]:  # pragma: no cover
        ...
        idx = Unfolding(Acc(self.State(), 1/1000), self._iof_idx)
        if not isinstance(idx, int):
            return None
        return cast(InfoOpaqueField, Unfolding(Acc(self.State(), 1/1000), self._ofs.get_by_idx(idx)))

    @Pure
    def get_hof(self) -> Optional[HopOpaqueField]:  # pragma: no cover
        Requires(Acc(self.State(), 1/1000))
        idx = Unfolding(Acc(self.State(), 1/1000), self._hof_idx)
        if not isinstance(idx, int):
            return None
        return cast(HopOpaqueField, Unfolding(Acc(self.State(), 1/1000), self._ofs.get_by_idx(idx)))

    @Pure
    def get_hof_ver(self, ingress: bool =True) -> Optional[HopOpaqueField]:
        Requires(Acc(self.State(), 1/1000))
        Requires(Unfolding(Acc(self.State(), 1/1000), isinstance(self._iof_idx, int) and isinstance(self._hof_idx, int)))
        """Return the :any:`HopOpaqueField` needed to verify the current HOF."""
        iof = self.get_iof()
        hof = self.get_hof()
        if Unfolding(Acc(self.State(), 1/1000), Unfolding(Acc(self._ofs.State(), 1/1000), Unfolding(Acc(hof.State(), 1/1000), Unfolding(Acc(iof.State(), 1/1000), not hof.xover or (iof.shortcut and not iof.peer))))):
        ...
        iof_peer = Unfolding(Acc(self.State(), 1/1000), Unfolding(Acc(self._ofs.State(), 1/1000), Unfolding(Acc(iof.State(), 1/1000), iof.peer)))
        iof_up_flag = Unfolding(Acc(self.State(), 1/1000), Unfolding(Acc(self._ofs.State(), 1/1000), Unfolding(Acc(iof.State(), 1/1000), iof.up_flag)))
        if iof_peer:
        ...
        if not isinstance(offset, int):
            return None
        return cast(HopOpaqueField, Unfolding(Acc(self.State(), 1/1000), self._ofs.get_by_idx(self._hof_idx + offset)))

    @Pure
    @ContractOnly
    def _get_hof_ver_normal(self, iof: InfoOpaqueField) -> Optional[HopOpaqueField]:
        Requires(Acc(self.State(), 1/1000))
        Requires(Unfolding(Acc(self.State(), 1/1000), Unfolding(Acc(self._ofs.State(), 1/1000), iof in self._ofs.contents())))
        # # Requires iof in bla
...
def valid_hof(path: SCIONPath) -> bool:
    Requires(Acc(path.State(), 1/1000))
    return True