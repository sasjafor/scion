...
class Router(SCIONElement):
    ...
    def send(self, packet: SCIONL4Packet, dst: HostAddrBase, dst_port: int):
        ...
        if from_local_as:
            result = self._remote_sock.send(packet.pack(), (str(dst), dst_port))
        else:
            result = self._udp_sock.send(packet.pack(), (str(dst), dst_port))
        return result[1]
    ...
    def handle_extensions(self, spkt: SCIONL4Packet, pre_routing_phase: bool, from_local_as: bool) -> List[Tuple[int, ...]]:
        ...
        assert len(spkt.ext_hdrs) == 0
        count = 0
        ext_hdrs = spkt.ext_hdrs
        for tmp in enumerate(ext_hdrs):
            i, ext_hdr = tmp
    ...
    def process_ifid_request(self, pkt, from_local):
        ...
        for addr in [a[0] for a in bs_addrs]:
            pkt.set_payload(ifid_pld.copy())
    ...
    def handle_data(self, spkt: SCIONL4Packet, from_local_as: bool, drop_on_error: bool=False):
        """
        ...
        try:
            return self._process_data(spkt, ingress, drop_on_error)
    ...
    def _process_data(self, spkt: SCIONL4Packet, ingress: bool, drop_on_error: bool):
        ...
        if ingress:
            ...
            return self.send(spkt, if_addr, port)
        else:
            path.inc_hof_idx()
            return self._egress_forward(spkt)
    ...
    def _process_flags(self, flags: List[Tuple[int, ...]], pkt, from_local_as):
        """
    ...
    def handle_request(self, packet, _, from_local_socket =True, sock =None):
        ...
        if not handler:
            return
        try:
            return self.handle_data(pkt, from_local_as)
        except SCMPError as e:
            self._scmp_validate_error(pkt, e)
        except SCIONBaseError:
